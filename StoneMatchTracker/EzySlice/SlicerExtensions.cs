...
namespace EzySlice {
    ...
    public static class SlicerExtensions {
        ...
        public static void ExtractMeshData(Mesh mesh, out Vector3[] vertices, out Vector3[] normals, out Vector2[] uvs)
        {
            var VertsList = new Il2CppSystem.Collections.Generic.List<Vector3>();
            mesh.GetVertices(VertsList);
            vertices = VertsList.ToArray();

            var NormalsList = new Il2CppSystem.Collections.Generic.List<Vector3>();
            mesh.GetNormals(NormalsList);
            normals = NormalsList.ToArray();

            var UvsList = new Il2CppSystem.Collections.Generic.List<Vector2>();
